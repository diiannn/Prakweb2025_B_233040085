<x-dashboard-layout>
    ...
    <div class="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-md dark:bg-gray-800">
        <h2 class="text-2xl font-bold mb-6 text-gray-800 dark:text-white">Create New Category</h2>
        ...
        <form action="{{ route('dashboard.categories.store') }}" method="post">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="name" name="name"
                    ...
                    placeholder="Ex: Web Programming" required value="{{ old('name') }}">
                @error('name')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500">{{ $message }}</p>
                @enderror
            </div>
            <button type="submit"
                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Create
                Category</button>
        </form>
    </div>
</x-dashboard-layout>